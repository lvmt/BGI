...
class HLA(object):
    ...
    def get_step4_shell(self):
        ...
        elif self.args['site_version'] == 'v5':
        ...
        {self.bin}/Rscript {self.bin}/{read_png} \\
    ...
    def start(self):
        ...
        for libid in self.info_dict.keys():
            ...
            jobcmd = 'qsub -e {logdir} -o {logdir}  -l vf={self.step1_memory},p=1 -P B2C_HLA {shellname}'.format(**locals())
            ...
            for i in range(1,97):
                ...
                jobcmd = 'qsub -e {logdir} -o {logdir}  -l vf=2g,p=1 -hold_jid {orders} -P B2C_HLA {shellname}'.format(**locals())
                self.can_qsub_job(jobcmd)
